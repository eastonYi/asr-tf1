...
from utils.dataset import PinyinDataSet
...
def train():
    dataset_train = PinyinDataSet(list_files=[args.dirs.text.data], args=args, _shuffle=True)
    ...
    dataset_supervise = PinyinDataSet(list_files=[args.dirs.text.supervise], args=args, _shuffle=True)
    tfdata_supervise = tf.data.Dataset.from_generator(
        dataset_supervise, (tf.int32, tf.int32), (tf.TensorShape([None]), tf.TensorShape([None])))
    ...
    dataset_dev = PinyinDataSet(list_files=[args.dirs.text.dev], args=args, _shuffle=False)
    ...
    with tf.train.MonitoredTrainingSession(config=config) as sess:
        ...
        text_len_supervise = pinyin_len_supervise = get_batch_length(pinyin_supervise)
        ...
        while global_step < 99999999:
            ...
            if global_step % args.dev_step == 1:
                pinyin_G_dev, text_G_dev = dev(iter_dev, tfdata_dev, dataset_dev, sess, G_infer)
...
def dev(iter_dev, tfdata_dev, dataset_dev, sess, G_infer):
    ...
    while 1:
        try:
            ...
            pinyin_G_dev, text_G_dev = sess.run(iter_dev)
            text_lens_G_dev = pinyin_lens_G_dev = get_batch_length(pinyin_G_dev)
            feature_dev, feature_lens_G_dev = int2vector(pinyin_G_dev, pinyin_lens_G_dev, hidden_size=args.model.dim_input, uprate=args.uprate)
    ...
    return pinyin_G_dev, text_G_dev


def decode(pinyin_G_dev, text_G_dev, sess, G_infer):
    pinyin_lens_G_dev = get_batch_length(pinyin_G_dev)
    feature_dev, feature_lens_G_dev = int2vector(pinyin_G_dev, pinyin_lens_G_dev, hidden_size=args.model.dim_input , uprate=args.uprate)
    feature_dev += np.random.randn(*feature_dev.shape)/args.noise
